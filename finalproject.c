...
typedef struct {
    ...
    int inStock;  // 1= in stock,  0= out of stock
} Order;
...
void flushInput() {
    int c;
    while ((c = getchar()) != '\n' && c != EOF); // read until it find "/n"(entre key) or end of the file. it stop
}
...
void addCustomer() {
    ...
    // Add new order
    if (b->orderCount >= MAX_ORDERS) {
        printf("Order limit reached for this customer.\n");
        return;
    }
